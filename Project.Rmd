...
## Detrending
The most obvious trend in the data, that must first be removed, is the seasonal trend. This trend can be seen in figure \ref{fig:seasonal} which shows the average snow depth during the differing months of the year. This graph has a clear sinusoidal shape, that peaks in X and reaches its lowest value in Y.
...
```{r, fig.cap="\\label{seasonal}Average snow depth by week of the year"}
...
detrended_collapsed <- detrended
# detrended_collapsed <- detrended[which(data$depth != 0)]
plot(detrended_collapsed, type="l")


# regr <-list(time=1:278)
regr <-list(time=time(detrended))

lin.mod <- lm(detrended_collapsed ~ time, data=regr)
summary(lin.mod)

plot(detrended_collapsed, type="l")
lines(predict(lin.mod, regr), type="l", col="red")

detrended_collapsed <- detrended_collapsed - predict(lin.mod, regr)
plot(detrended_collapsed, type="l")

detrended <- detrended_collapsed


# depth2 <- data$depth/a2$x[match(as.numeric(format(data$date + 10*4, "%Y")), a2$period)]
# plot(data$date, data$depth, type="l")

# plot(data$date, depth2, type="l")
# 
# agg3 <- aggregate(
#   depth2,
#   by=list(period = periods),
...
# )
# 
# plot(1:range(periods)[2], agg3$x)
# 
# depth3 <- depth2 - agg3$x[match(periods, agg3$period)]
# plot(periods, depth3)
# plot(data$date, depth3, type="l")
# 
# depth4 <- depth3[which(depth3 != 0)]
# plot(depth4, type="l")
# mvspec(depth4, log="no")
# 
# detrended <- depth4 - mean(depth4)

#k = kernel("daniell", c(1,1))
# pg <- mvspec(diff(detrended), kernel=k, log="no")
# pg <- mvspec(depth4, kernel=k, log="no")

# pg <- mvspec(depth4, log="no")
```
